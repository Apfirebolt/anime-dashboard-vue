  ...
  <div class="h-full flex flex-col">
    ...
    <header class="flex-shrink-0 relative h-16 bg-white flex items-center">
      ...
      >
        <div class="min-w-0 flex-1">
          ...
          >
            <label for="desktop-search" class="sr-only">Search People</label>
            <input
              id="desktop-search"
              type="search"
    ...
    <div class="min-h-0 overflow-hidden">
      ...
      <main class="min-w-0 border-t border-gray-200">
          ...
          <div class="flex flex-col gap-6 md:flex-row items-center max-w-8xl">
            <div class="w-full md:w-1/2 lg:pr-32">
              ...
              >
                ...
                It has a lot of useful end-points to be used to call. On this page you can search for People.
              </h3>
              <p
                v-if="peopleList && peopleList.data && peopleList.data.length > 0"
              >
        ...
        >
          <Pagination
            v-if="peopleList && peopleList.pagination"
            :total-pages="peopleList.pagination.last_visible_page"
            :current-page="peopleList.pagination.current_page"
            :next-page="peopleList.pagination.has_next_page"
          ...
          >
            <!-- Thread section-->
            <div v-if="peopleList && peopleList.data" class="columns-1 sm:columns-2 lg:columns-3 gap-4 p-4">
              <div
                v-for="item in peopleList.data"
                :key="item.id"
...
const searchPeopleUtil = async () => {
  people.searchPeopleAction(searchText.value);
};

const peopleList = computed(() => people.getPeopleList);
const isLoadingPeopleList = computed(() => people.isLoading);
...
const onPageChange = async (page) => {
  ...
  await people.searchPeopleAction(searchText.value, page);
};
