<template>
  <div class="h-full flex flex-col bg-gray-50">
    <!-- Top nav-->
    <header
      class="flex-shrink-0 relative h-16 bg-white flex items-center shadow-sm border-b border-gray-200"
    >
      <!-- Logo area -->
      <Logo />
      <!-- Menu button area -->
      ...
      >
        ...
        <button
          type="button"
          class="-mr-2 inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-600 transition-colors"
          @click="open = true"
        >
    ...
    <div class="min-h-0 overflow-hidden">
      ...
      <main class="min-w-0 border-t border-gray-200">
        ...
        >
          ...
          >
            <!-- Thread section-->
            <ul role="list" class="py-6 space-y-6 sm:px-6 lg:px-8">
              <li
                v-for="item in animeList.data"
                :key="item.id"
                class="bg-white border border-gray-200 hover:border-blue-300 transition-all duration-200 overflow-hidden shadow-sm hover:shadow-md sm:rounded-lg"
              >
                <div class="p-6">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      ...
                      >
                        ...
                        <span v-if="item.year" class="text-gray-600">{{
                          item.year
                        }}</span>
                      </div>
                    </div>
                    <div v-if="item.images?.jpg" class="ml-4 flex-shrink-0">
                      <img
                        :src="item.images.jpg.small_image_url"
                        :alt="item.title"
                        class="h-32 w-48 object-cover rounded shadow-sm"
                      />
                    </div>
                  </div>
